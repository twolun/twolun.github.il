...
function removeNode(){
	...
	for(var i = 0;i<arr.length;i++){
		var node = arr[i];
		if(node.nodeName=="SCRIPT" && node.outerHTML.indexOf('jquery.min.js')<0 && node.outerHTML.indexOf('high_chemistry')<0 && node.outerHTML.indexOf('preload.min.js')<0 && node.outerHTML.indexOf('custom.js')<0){
			node.remove();
		}
		if(node.nodeName=="LINK" && node.outerHTML.indexOf('high_chemistry')<0  && node.outerHTML.indexOf('custom.css')<0 && node.outerHTML.indexOf('preview.css')<0 && node.outerHTML.indexOf('preload.min.css')<0){
			node.remove();
		}
	}
}
